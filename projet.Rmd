...
Quand on effectue la régression RIDGE avec la commande `glmnet`, on obtient un objet contenant plusieurs informations. Nous n'avons pas spécifié de valeur de $\lambda$, cet objet va donc contenir 100 valeurs de $\lambda$ différentes. Pour chacun de ces $\lambda$, nous allons avoir les coefficients associés après la pénalisation. 
...
```{r}
# graphique de la déviance binomiale en fonction des log lambda
plot(cv_ridge)
...
conf_matrix_ridge <- evaluate_model(test_set$Ybin, pred_classes_ridge)
```
...
La régression LASSO va avoir tendance à mettre des coefficients à 0 et donc à faire de la sélection de variables. 
...
# Affichage des prédictions
result_lasso <- data.frame(Prédiction = pred_classes_lasso, Probabilités = pred_probs_lasso)
result_lasso
```
...
# courbe ROC
plot(roc_perf_lasso, colorize = TRUE, main = "Courbe ROC - Ensemble de test", print.cutoffs.at = seq(0, 1,by = 0.1), text.adj = c(1.2, 1.2), lwd = 3)
```

```{r}
# AUC
auc_lasso <- performance(pred_lasso, "auc")@y.values[[1]]
cat("AUC sur l'ensemble de test :", auc_lasso, "\n")
```
...
```{r}
# Création de la matrice de confusion
conf_matrix_lasso <- addmargins(table(Prédiction = pred_classes_lasso, Réalité = test_set$Ybin))
...
```{r}
# Utilisation de la fonction
conf_matrix_lasso <- evaluate_model(test_set$Ybin, pred_classes_lasso)
```
